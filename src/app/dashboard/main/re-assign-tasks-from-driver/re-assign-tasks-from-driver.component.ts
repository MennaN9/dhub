...
@Component({
  selector: 'app-re-assign-tasks-from-driver',
  templateUrl: './re-assign-tasks-from-driver.component.html',
  styleUrls: ['./re-assign-tasks-from-driver.component.scss']
})
export class ReAssignTasksFromDriverComponent implements OnInit {
  ...
  onSelectTeam() {
    this.teamsSelectList.valueChange.subscribe(id => {
      ...
      if (this.drivers.length == 0) {
        this.snackBar.openSnackBar({ message: this.translateService.instant('No drivers found at that team'), duration: 2500, action: this.translateService.instant('Okay') });
      }
    });
  }
